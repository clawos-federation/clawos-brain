...
class TaskDispatcher {
  ...
  async execute(task, routingPlan, options = {}) {
    ...
    if (!routingPlan) {
      console.error('[TaskDispatcher] Missing routingPlan');
      return {
        executionId: `exec_${Date.now()}`,
        error: 'Routing plan is required',
        success: false
      };
    }
    ...
    if (!agents || !Array.isArray(agents) || agents.length === 0) {
      console.error('[TaskDispatcher] No agents available in routing plan');
      return {
        executionId: `exec_${Date.now()}`,
    ...
    if (!mode) {
      console.error('[TaskDispatcher] Missing execution mode in routing plan');
      return {
        executionId: `exec_${Date.now()}`,
        error: 'Execution mode is required',
        success: false
      };
    }
    ...
    try {
      ...
      switch (mode) {
        case 'single-agent':
          if (!agents[0]) {
            throw new Error('No agent available for single-agent mode');
          }
        ...
        default:
          console.error(`[TaskDispatcher] Unknown execution mode: ${mode}`);
      ...
      try {
        ...
        console.error('[TaskDispatcher] Failed to log execution:', logError);
        // 不影响执行结果
      }
  ...
  async executeAgentTask(task, agent, executionId) {
    // Simulate execution (real implementation would call LLM)
